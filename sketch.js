...
function setup() {
  canvass = createCanvas(windowWidth, windowHeight / 2);
  cx = (windowWidth - width) / 2;
  cy = (windowHeight - height) / 2;
  canvass.position(cx, cy);
  //width es el ancho y height es el alto del canvas
  fondo = createSprite(width/2, height / 2);
  fondo.addImage("desierto", desierto);
  fondo.scale = 1.8;
  fondo.velocityX = -5.2;

  piso_invisible = createSprite(50, height - 52, 50, 2);
...
function sky() {
  if (frameCount % 140 === 0) {
    cloud = createSprite(width, random(height - 260,height-210));
...
function plantas() {
  ...
  if (frameCount % 182 === 0) {
    cactus = createSprite(width,random(height-90,height-85))
